...
# ============================================================
# 2.5. CONFIGURAR /dev/shm (evitar problemas de memória)
# ============================================================
...
# ============================================================
# 2.6. CORRIGIR HOSTNAME NO /ETC/HOSTS
# ============================================================
print_header "2.6. CORRIGINDO HOSTNAME NO /ETC/HOSTS"
...
# ============================================================
# 5.5. VERIFICAR /dev/shm ANTES DE INICIAR POSTGRESQL
# ============================================================
...
#!/usr/bin/env python3
# /opt/cgnat/cgnat_parser.py - VERSÃO OTIMIZADA COM CACHE E IPv6
...
print_success "Parser Python criado com otimizações"
...
# ============================================================
# 12.10 CONSULTAR.PHP (VERSÃO CORRIGIDA - IPv6 INDEPENDENTE)
# ============================================================
...
// ============================================================
// FUNÇÃO PARA IDENTIFICAR TIPO DE IP
// ============================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($ip_publico) && empty($ipv6_busca)) {
        ...
        try {
            ...
            // ============================================================
            // CASO 1: CONSULTA POR IPv4 (CGNAT)
            // ============================================================
...
<body>
    <div class="container">
        ...
        <?php if ($cliente_encontrado && $cliente_nome && $cliente_nome != 'Nao identificado' && $cliente_nome != 'NAO IDENTIFICADO - Cliente sem IPv6 no Cisco'): ?>
        ...
        <div class="results">
            <h3>
                ...
                <?php if ($cliente_encontrado && $cliente_nome != 'NAO IDENTIFICADO - Cliente sem IPv6 no Cisco'): ?>
...
if [ $USO -gt 90 ]; then
    ...
    rm -rf /dev/shm/PostgreSQL.* 2>/dev/null
    rm -rf /dev/shm/sem.* 2>/dev/null
    systemctl restart postgresql@15-main 2>/dev/null || systemctl restart postgresql@17-main 2>/dev/null
fi
EOF
...
# ============================================================
print_header "18. AJUSTANDO PERMISSÕES FINAIS"
...
echo "   4. Execute a sincronização IPv6: /usr/local/bin/sync_ipv6_cisco.sh"
echo ""
echo "============================================================"
